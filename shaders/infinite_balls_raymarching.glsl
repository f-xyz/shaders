...
vec2 world(vec3 v) {
    ...
    float r = 0.3;
    //float s = abs(sin(time*4.0));
    //float el = 2.0-abs(sin(time*4.0));
    float repl = 1.0;
    vec3 replV = mod(vec3(v.x, v.y+0.1, v.z), repl) - 0.5*repl;
    return //join(
        vec2(sphere(replV, r), 1.0);//,
        //vec2(plane(v, r), 0.5));
}
...
void main() {
    ...
    float dist = 1.0;
    float a = 10.0*time;
    eye.x += dist*cos(a*0.75);
    eye.y += 1.5+dist*abs(sin(a*0.5));
    eye.z += dist*sin(a*0.35);
