...
void main() {
    ...
    vec3 prp = vec3(0.0, 0.0, 1.0); // from
    ...
    vec3 scp = normalize(scrCoord - prp); // direction

    //
    const vec3 e = vec3(0.1, 0.0, 0.0); // gradient delta
    const float maxDepth = 100.0;
    // intersection?
    vec2 s = vec2(0.1, 0.0); // x -> total distance, y -> material
    vec3 c, p, n;

    float f = 1.0; // total distance
    for (int i=0; i<256; i++) {
